...
export async function renameChannel(args: {
  ...
  if (user.role !== "admin" && channel.created_by !== user.id) {
    return { ok: false, code: 403, error: "You don't have permission to rename this channel" };
  }
...
export async function sendMessage(args: {
  ...
  if (args.replyToId) {
    ...
    const parentMessage = replyRows[0] as any;
    if (!parentMessage) {
      return { ok: false, code: 404, error: "Reply target not found" };
    }

    if (parentMessage.channel_id !== args.channelId) {
      return { ok: false, code: 400, error: "Reply target is in a different channel" };
    }
  }
...
export async function updateEvent(args: {
  ...
  const rows = await sql`
    ...
    LIMIT 1
  `;
  const event = rows[0] as { id: string; created_by: string } | undefined;
  if (!event) {
    return { ok: false, code: 404, error: "Event not found" };
  }

  if (event.created_by !== user.id && user.role !== "admin") {
    return { ok: false, code: 403, error: "Not authorized to update this event" };
  }
...
export async function deleteEvent(args: {
  ...
  if (event.created_by !== user.id && user.role !== "admin") {
    return { ok: false, code: 403, error: "Not authorized to delete this event" };
  }

  await sql`DELETE FROM event_attendees WHERE event_id = ${args.eventId}`;
  await sql`DELETE FROM calendar_events WHERE id = ${args.eventId}`;
