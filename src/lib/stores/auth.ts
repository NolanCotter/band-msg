...
function createAuthStore() {
  ...
  async function updatePresenceSilently(status: 'online' | 'idle' | 'dnd' | 'offline') {
    try {
      const response = await apiPost('/api/presence', { status });
      if (!response.ok) {
        console.warn('[authStore] Presence update failed with status', response.status);
      }
    } catch (error) {
      console.warn('[authStore] Presence update failed', error);
    }
  }
  ...
  return {
    ...
    async checkAuth() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          void updatePresenceSilently('online');
        } else {
    ...
    async logout() {
      await updatePresenceSilently('offline');
      try {
