<script lang="ts">
  ...
  async function renameChannel() {
    ...
    try {
      const response = await apiPatch(`/api/channels/${menuChannel.id}`, {
      ...
      if ($convexChannelStore.selectedChannelId === menuChannel.id) {
        convexChannelStore.selectChannel(menuChannel.id);
      }
      isRenaming = false;
