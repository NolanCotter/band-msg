<script lang="ts">
  ...
  function getDraftStorageKey(channelId: string) {
    const sessionScope = $authStore.user?.id || $authStore.user?.username || 'guest';
    return `band-chat:draft:v1:${sessionScope}:${channelId}`;
  }

  function readDraft(channelId: string) {
    if (!canUseDraftStorage()) return '';

    try {
      return window.localStorage.getItem(getDraftStorageKey(channelId)) || '';
    } catch {
      return '';
    }
  }
  ...
  function writeDraft(channelId: string, value: string) {
    ...
    try {
      const key = getDraftStorageKey(channelId);
      if (value.trim()) {
  ...
  function flashDraftSaved(message: string) {
    draftSavedMessage = message;
    if (draftSavedTimer) {
  ...
  $: {
    ...
    if (selectedChannelId !== draftChannelId) {
      draftChannelId = selectedChannelId;
      messageInput = selectedChannelId ? readDraft(selectedChannelId) : '';
      showMentionDropdown = false;
      tick().then(syncComposerHeight);
    }
  }
  ...
  async function handleSend() {
    ...
    if (result.success) {
      messageInput = '';
      writeDraft($convexChannelStore.selectedChannelId, '');
      flashDraftSaved('Message sent');
      showMentionDropdown = false;
      vibrateSuccess();
      syncComposerHeight();
  ...
  function handleInput(e: Event) {
    ...
    if (selectedChannelId) {
      writeDraft(selectedChannelId, value);
    }
  ...
  function clearCurrentDraft() {
    ...
    messageInput = '';
    writeDraft(selectedChannelId, '');
    showMentionDropdown = false;
    syncComposerHeight();
    flashDraftSaved('Draft cleared');
  }
  ...
  async function retryCurrentConversation() {
    const selectedChannelId = $convexChannelStore.selectedChannelId;
    if (!selectedChannelId) return;

    await messageStore.loadMessages(selectedChannelId);
  }
  ...
  async function renameMobileChannel() {
    ...
    try {
      const response = await apiPatch(`/api/channels/${mobileMenuChannel.id}`, {
        name: mobileNewChannelName.trim(),
      });

      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        throw new Error(typeof body.error === 'string' ? body.error : 'Failed to rename channel');
      }
  ...
  async function deleteMobileChannel() {
    ...
    try {
      const response = await apiPost('/api/channels/delete', {
        channelId: mobileChannelToDelete.id,
      });

      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        throw new Error(typeof body.error === 'string' ? body.error : 'Failed to delete channel');
      }
...
>
  ...
  >
    ...
    {:else if $convexChannelStore.error && $convexChannelStore.channels.length === 0}
      <div class="min-h-full flex items-center justify-center px-6">
        <div class="max-w-sm text-center">
          <div class="text-sm font-medium text-rose-200">Channel sync failed</div>
          <p class="mt-1 text-xs text-white/45">{$convexChannelStore.error}</p>
          <div class="mt-4 flex flex-col sm:flex-row items-center justify-center gap-2">
            <button
              type="button"
              on:click={retryChannelStartup}
    ...
    {:else if $messageStore.error && $messageStore.messages.length === 0}
      <div class="min-h-full flex items-center justify-center px-6">
        <div class="max-w-sm text-center">
          <div class="text-sm font-medium text-rose-200">Conversation unavailable</div>
          <p class="mt-1 text-xs text-white/45">{$messageStore.error}</p>
          <button
            type="button"
            on:click={retryCurrentConversation}
            class="mt-4 px-4 py-2 rounded-xl bg-white text-black text-sm font-semibold hover:bg-white/90 transition-colors"
          >
            Reload conversation
          </button>
        </div>
  ...
  <div class="message-composer px-4 pt-2 shrink-0 border-t border-white/10 bg-black/95 backdrop-blur-sm relative z-20 animate-slide-up" style="padding-bottom: calc(env(safe-area-inset-bottom) + 0.75rem);">
    <div class="mb-2 flex items-center justify-between gap-3 text-[11px] text-white/35">
      <span>{draftSavedMessage || (messageInput.trim() ? 'Draft saved automatically' : 'Tip: drafts are saved per channel')}</span>
      {#if messageInput.trim()}
        <button
          type="button"
          on:click={clearCurrentDraft}
          class="rounded-full border border-white/10 px-2.5 py-1 text-[11px] font-medium text-white/55 transition-colors hover:bg-white/5 hover:text-white"
        >
          Clear draft
        </button>
      {/if}
...
{#if showAdminPanel}
  <AdminPanel onClose={() => { showAdminPanel = false; scrollToBottom(true); }} />
  {/if}
