<script lang="ts">
  ...
  async function handleCreate() {
    ...
    try {
      const response = await apiPost('/api/channels', {
        name: cleanName,
      ...
      if (!response.ok) {
        ...
        throw new Error(typeof body.error === 'string' ? body.error : 'Failed to create channel');
      }
  ...
  <Drawer.Portal>
    ...
    <Drawer.Content class="fixed bottom-0 left-0 right-0 md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:bottom-auto md:right-auto z-[200] flex flex-col bg-black rounded-t-[20px] md:rounded-2xl max-h-[92vh] md:max-h-[85vh] md:w-full md:max-w-lg outline-none" style="padding-bottom: env(safe-area-inset-bottom);">
      ...
      <div class="flex items-center justify-between px-4 py-3 border-b border-white/10 shrink-0 md:px-6 md:py-4">
        <h2 class="text-lg font-bold text-white md:text-xl">Create Channel</h2>
        <button
          type="button"
          on:click={onClose}
...
{#if showMemberSelector}
  <Drawer.Root open={true} onOpenChange={(open) => { if (!open) showMemberSelector = false; }}>
    <Drawer.Portal>
      <Drawer.Overlay class="fixed inset-0 bg-black/80 z-[300]" />
      <Drawer.Content class="fixed bottom-0 left-0 right-0 md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:bottom-auto md:right-auto z-[300] flex flex-col bg-black rounded-t-[20px] md:rounded-2xl max-h-[92vh] md:max-h-[85vh] md:w-full md:max-w-lg outline-none" style="padding-bottom: env(safe-area-inset-bottom);">
        <div class="mx-auto w-12 h-1.5 flex-shrink-0 rounded-full bg-white/10 my-3 md:hidden"></div>
        <MemberSelector 
