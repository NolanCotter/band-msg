  ...
  async function refreshMessages() {
    ...
    try {
      ...
      if (messageIds.length > 0) {
        const reactionsRes = await apiGet(`/api/reactions?messageIds=${messageIds.map(id => encodeURIComponent(id)).join(',')}`);
        if (reactionsRes.ok) {
      ...
      // Auto-scroll to bottom on initial load or if user was already at bottom
    ...
    <section class="discord-layout">
      ...
      <section class="chat-main">
        ...
        {#if showNewMessagesBar}
          <button class="new-messages-bar" on:click={jumpToBottom}>
            <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M12 20v-8h-6l8-8v8h6z"/></svg>
            {newMessagesCount} new {newMessagesCount === 1 ? 'message' : 'messages'}
          </button>
        {/if}

        <div class="messages-scroll" bind:this={messageContainer} on:scroll={handleScroll}>
